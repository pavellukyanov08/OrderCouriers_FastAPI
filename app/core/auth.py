...
async def get_user(db: AsyncSessionLocal, username: str) -> Optional[User]:
    result = await db.execute(select(User).where(User.username == username))
    return result.scalar_one_or_none()


async def authenticate_user(db: AsyncSessionLocal, username: str, password: str) -> Optional[User]:
